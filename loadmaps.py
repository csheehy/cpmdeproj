...
if doload:
    ...
    suff = '006_TR1.2++TR10.0+pol_alpha1_cpmlr_alldk++alldk'
...
#a = aps.aps(mn,msn,ext2='pred_cpm')
#app = aps.aps(msn,msn,ext='pred',ext2='pred_cpm')

a2 = aps.aps(msn,msn,mb=me,ext2='pred_cpm')
...
plot(a.l,a2.dl[2][2],'-x',color='gray',label='<SAT>x<pred>')
plot(a.l,a.dl[2][2],'-xk',lw=2,label='<SAT-LAT>x<pred>')
#plot(a.l,app.dl[2][2],'-xg',lw=2,label='<pred0>x<pred>')
plot(l, l*(l+1)*r0p1*r/0.1/(2*np.pi), '--k',label='r={:0.0e}'.format(r))
